...
interface DevDrnnAuxWorkspaceProps {
  darkMode: boolean;
  mode?: 'run' | 'tune';
  onBack: () => void;
}

interface DevDrnnAuxResult {
  status: string;
  dataset_name: string;
  method?: string;
  backend?: string;
...
export default function DevDrnnAuxWorkspace({ darkMode, mode = 'run', onBack }: DevDrnnAuxWorkspaceProps) {
  ...
  // DevDRNN-Aux Execution Config parameters
  const [numberOfTests, setNumberOfTests] = useState<number>(20);
  const [seed, setSeed] = useState<boolean>(true);
  const [subsetPercentage, setSubsetPercentage] = useState<number>(0.1);
  const [numAuxNet, setNumAuxNet] = useState<number>(1);
  ...
  // DevDRNN-Aux Tuning Config parameters
  ...
  const [spMin, setSpMin] = useState<number>(0.1);
  const [spMax, setSpMax] = useState<number>(0.9);
  const [numAuxMin, setNumAuxMin] = useState<number>(1);
  const [numAuxMax, setNumAuxMax] = useState<number>(5);
  ...
  const savedTaskId = localStorage.getItem(`dev_drnn_aux_active_task_id_${mode}`);
  const savedStatus = savedTaskId ? 'running' : 'idle';
  const savedProgressPercent = Number(localStorage.getItem(`dev_drnn_aux_progress_percent_${mode}`)) || 0;
  const savedProgressMsg = localStorage.getItem(`dev_drnn_aux_progress_msg_${mode}`) || '';

  const savedCompletedTestsCount = Number(localStorage.getItem('dev_drnn_aux_completed_tests_count')) || 1;
  ...
  const [results, setResults] = useState<DevDrnnAuxResult | null>(null);
  ...
  const clearActiveTask = () => {
    setTaskId(null);
    localStorage.removeItem(`dev_drnn_aux_active_task_id_${mode}`);
    localStorage.removeItem(`dev_drnn_aux_progress_percent_${mode}`);
    localStorage.removeItem(`dev_drnn_aux_progress_msg_${mode}`);
  };
  ...
  const getPlotUrls = (outputFile: string, cycleIndex: number, runDataset: string, runMethod: string) => {
    ...
    return {
      train: `http://localhost:8004/static/${relativeFolder}/confusion_matrix/${trainImgName}`,
      test: `http://localhost:8004/static/${relativeFolder}/confusion_matrix/${testImgName}`
    };
  };
  ...
  useEffect(() => {
    ...
    let statusUrl = `http://localhost:8004/nn/dev_drnn_aux/status/${taskId}`;
    if (mode === 'tune') {
      statusUrl = `http://localhost:8004/nn/dev_drnn_aux/tune/status/${taskId}`;
    }
      ...
      try {
        ...
        if (response.ok) {
          const data = (await response.json()) as {
            status: string;
            info: DevDrnnAuxResult | ProgressInfo | string | null
          };

          if (data.status === 'SUCCESS') {
            setStatus('idle');
            setProgressPercent(100);
            setResults(data.info as DevDrnnAuxResult);
            setSelectedCycle(0);

            if (mode === 'run') {
              const runTests = Number(localStorage.getItem('dev_drnn_aux_active_tests_count')) || numberOfTests;
              setCompletedTestsCount(runTests);
              localStorage.setItem('dev_drnn_aux_completed_tests_count', String(runTests));
            }

            clearActiveTask();
            localStorage.removeItem('dev_drnn_aux_active_tests_count');
            if (pollingRef.current) clearInterval(pollingRef.current);
          } else if (data.status === 'FAILURE') {
            setStatus('error');
            setErrorMessage(typeof data.info === 'string' ? data.info : 'Unknown DevDRNN-Aux engine error.');
            clearActiveTask();
            localStorage.removeItem('dev_drnn_aux_active_tests_count');
            if (pollingRef.current) clearInterval(pollingRef.current);
            ...
            if (mode === 'tune') {
              ...
              newMsg = progressInfo.status || 'Iterative Pruning & Training...';
            ...
            localStorage.setItem(`dev_drnn_aux_progress_percent_${mode}`, String(newPercent));
            localStorage.setItem(`dev_drnn_aux_progress_msg_${mode}`, newMsg);
          }
        }
      } catch (err) {
  ...
  const handleStart = async () => {
    ...
    let startUrl = `http://localhost:8004/nn/dev_drnn_aux/start?dataset_name=
    ${datasetName}&method=${method}&activation=${activation}`;
    let payload: Record<string, unknown> = {
      number_of_tests: numberOfTests,
      seed: seed,
      subset_percentage: subsetPercentage,
      num_aux_net: numAuxNet,
    ...
    if (mode === 'tune') {
      startUrl = `http://localhost:8004/nn/dev_drnn_aux/tune?dataset_name=${datasetName}`;
      payload = {
        backend,
        n_trials: nTrials,
        seed: seed,
        ...
        sp_min: spMin,
        sp_max: spMax,
        num_aux_min: numAuxMin,
        num_aux_max: numAuxMax,
        l1_min: l1Min,
        l1_max: l1Max,
        l2_min: l2Min,
        l2_max: l2Max,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.detail || 'Failed to start DevDRNN-Aux process.');
      }

      const data = await response.json() as { task_id?: string };
      if (data.task_id) {
        setTaskId(data.task_id);
        localStorage.setItem(`dev_drnn_aux_active_task_id_${mode}`, data.task_id);
        localStorage.setItem('dev_drnn_aux_active_tests_count', String(numberOfTests));
        localStorage.setItem(`dev_drnn_aux_progress_percent_${mode}`, '0');
        localStorage.setItem(`dev_drnn_aux_progress_msg_${mode}`, 'Queuing task in Celery...');
      }
    } catch (err) {
      setStatus('error');
  ...
  const handleStop = async () => {
    if (!taskId) return;
    try {
      let stopUrl = `http://localhost:8004/nn/dev_drnn_aux/stop/${taskId}`;
      if (mode === 'tune') {
        stopUrl = `http://localhost:8004/nn/dev_drnn_aux/tune/stop/${taskId}`;
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <span className="text-xs font-mono text-emerald-500 uppercase tracking-wider">
          DevDRNN-Aux {mode === 'tune' ? 'Hyperparameter Tuning Lab' : 'Workspace'}
        </span>
      </div>
        ...
        <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} space-y-5 flex flex-col justify-between h-full shadow-md`}>
              ...
              {mode === 'tune' ? (
                ...
                <div className={`pt-2 border-t space-y-4 ${darkMode ? 'border-slate-800/40' : 'border-slate-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className={`block text-[11px] font-medium mb-1 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>Subset % Min / Max</label>
                      <div className="flex gap-1">
                        <input
                          ...
                          value={spMin}
                          disabled={status === 'running'}
                          onChange={(e) => setSpMin(parseFloat(e.target.value) || 0.01)}
                        ...
                        <input
                          ...
                          value={spMax}
                          disabled={status === 'running'}
                          onChange={(e) => setSpMax(parseFloat(e.target.value) || 0.99)}
                          className={`w-1/2 text-xs p-1.5 rounded border font-mono ${
                            darkMode ? 'bg-slate-950 border-slate-700 text-slate-200' : 'bg-white border-slate-300 text-slate-800'
                          }`}
                        />
                    ...
                    <div>
                      <label className={`block text-[11px] font-medium mb-1 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>Num Aux Min / Max</label>
                      <div className="flex gap-1">
                        <input
                          type="number"
                          value={numAuxMin}
                          disabled={status === 'running'}
                          onChange={(e) => setNumAuxMin(parseInt(e.target.value) || 1)}
                          className={`w-1/2 text-xs p-1.5 rounded border font-mono ${
                            darkMode ? 'bg-slate-950 border-slate-700 text-slate-200' : 'bg-white border-slate-300 text-slate-800'
                          }`}
                        />
                        <input
                          ...
                          value={numAuxMax}
                          disabled={status === 'running'}
                          onChange={(e) => setNumAuxMax(parseInt(e.target.value) || 1)}
                          className={`w-1/2 text-xs p-1.5 rounded border font-mono ${
                            darkMode ? 'bg-slate-950 border-slate-700 text-slate-200' : 'bg-white border-slate-300 text-slate-800'
                          }`}
                        />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setSubsetPercentage(parseFloat(e.target.value) || 0.1)}
                        className={`w-full text-sm p-2 rounded-md border ${
                          darkMode ? 'bg-slate-950 border-slate-700 text-slate-200' : 'bg-white border-slate-300 text-slate-800'
                        } disabled:opacity-50`}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        disabled={status === 'running'}
                        value={numAuxNet}
                        onChange={(e) => setNumAuxNet(parseInt(e.target.value) || 1)}
                        className={`w-full text-sm p-2 rounded-md border ${
                          darkMode ? 'bg-slate-950 border-slate-700 text-slate-200' : 'bg-white border-slate-300 text-slate-800'
                        } disabled:opacity-50`}
                      />
          ...
          <div className="pt-4 space-y-2">
            {status === 'running' ? (
              ...
              >
                <Square className="w-4 h-4 fill-white" /> Stop DevDRNN-Aux Process
              </button>
            ) : (
              <button
                onClick={handleStart}
              ...
              >
                <Play className="w-4 h-4 fill-white" /> {mode === 'tune' ? 'Launch Hyperparameter Search' : 'Execute DevDRNN-Aux Run'}
              </button>
            )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h4 className={`font-semibold text-sm ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                    {mode === 'tune' ? 'Optimizing DevDRNN-Aux hyperparameters...' : 'DevDRNN-Aux is running...'}
                  </h4>
                </div>
            ...
            {mode === 'run' && results && plotUrls ? (
              <div className="flex flex-col h-full w-full justify-between space-y-4">
                ...
                <div className={`flex-1 flex items-center justify-center relative group border rounded-xl overflow-hidden bg-white p-4 shadow-inner min-h-[300px] max-h-[520px] ${
                  ...
                  <img
                    src={activeImageUrl}
                    alt="DevDRNN-Aux Confusion Matrix"
            ...
            {status === 'idle' && !results && (
              <div className="text-center space-y-2 py-12">
                ...
                <p className={`text-xs max-w-xs ${darkMode ? 'text-slate-500' : 'text-slate-600'}`}>
                  {mode === 'tune' ? 'Configure search limits and launch DevDRNN-Aux tuning.' : 'Run the DevDRNN-Aux network to generate and display the pruning phase-plots.'}
                </p>
              </div>
            )}
          </div>
      ...
      {isZoomed && activeImageUrl && (
        ...
        >
          ...
          <div className="max-w-[95vw] max-h-[90vh] flex flex-col items-center space-y-3" onClick={(e) => e.stopPropagation()}>
            <img
              src={activeImageUrl}
              alt="DevDRNN-Aux Fullscreen Confusion Matrix"
