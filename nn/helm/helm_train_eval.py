...
from utils.utils import (average_columns_in_excel, create_timestamp, insert_data_to_excel, reorder_metrics_lists,
                         plot_confusion_matrices_fcnn_helm)
...
class HELM(HELMBase):
    def __init__(self):
        ...
        helm_config = helm_paths_config(self.dataset_name)

        self.filename = \
            os.path.join(
                helm_config.get("path_to_results"),
                f"{timestamp}_{self.dataset_name}_dataset.xlsx"
            )

        self.save_path = os.path.join(helm_config.get("path_to_cm"), timestamp)
        os.makedirs(self.save_path, exist_ok=True)

        self.hyperparam_config = {
            "C_penalty":
                self.cfg.get("penalty"),
            "scaling_factor":
                self.cfg.get("scaling_factor"),
        }
    ...
    def main(self) -> None:
        ...
        for idx in tqdm(range(self.cfg.get("num_tests")), desc="Evaluation"):
            t3, beta, beta1, beta2, l3, ps1, ps2 = (
                self.train(
            ...
            plot_confusion_matrices_fcnn_helm(
                cm_train=training_metrics[-1],
                cm_test=testing_metrics[-1],
                class_labels=self.labels,
                dataset_name=self.dataset_name,
                path_to_plot=self.save_path,
                index=idx
            )

            # Save test results into xlsx
            metrics = (
            ...
            insert_data_to_excel(
                self.filename, self.dataset_name, idx + 2, metrics
            )
