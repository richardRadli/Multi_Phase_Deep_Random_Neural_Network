...
from services.dev_drnn_aux_service.tasks import celery_app, ipmpdrnn_task, ipmpdrnn_tune_task

dev_drnn_aux_router = APIRouter(prefix="/nn/dev_drnn_aux", tags=["DevDRNN-Aux Unified Control"])
...
class DevDRNNAuxConfig(BaseModel):
    ...
    subset_percentage: float = Field(default=0.1, ge=0.01, le=0.99, description="Metszési arány (Subset Percentage)")
    num_aux_net: int = Field(default=1, ge=1, le=10, description="Segédhálózatok száma (Num Aux Nets)")

    sigma: Optional[float] = Field(default=0.1, ge=0.01, le=1.0, description="Szórás (BASE esetén figyelmen kívül hagyva)")
    penalty: Optional[float] = Field(default=None, description="L2 penalty felülírás (Kizárólag EXP_ORT_C-nél megengedett)")
    ...
    hidden_neurons: Optional[List[int]] = Field(default=None, description="Manuálisan megadott rétegenkénti neuronszámok [L1, L2, L3]")
...
class DevDRNNAuxTuneConfig(BaseModel):
    ...
    sp_min: float = Field(default=0.1, ge=0.01, le=0.99, description="Subset Percentage minimum")
    sp_max: float = Field(default=0.9, ge=0.01, le=0.99, description="Subset Percentage maximum")
